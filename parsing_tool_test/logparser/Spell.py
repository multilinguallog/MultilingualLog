...
class LogParser:
    ...
    def log_to_dataframe(self, log_file, regex, headers, logformat):
        ...
        with open(log_file, "r") as fin:
            for line in fin.readlines():
                """Have no idea what this statement want to do"""
    ...
    def outputResult_nonEnglish(self, logClustL):
    ...
    def parse_nonEnglish(self, logname):
        starttime = datetime.now()
        print("Parsing file: " + os.path.join(self.path, logname))
        self.logname = logname
        self.load_data()
        ...
        for idx, line in self.df_log.iterrows():
            logID = line["LineId"]
            logmessageL = list(jieba.cut(self.preprocess(line["Content"])))
            constLogMessL = [w for w in logmessageL if w != "<*>"]
        ...
        self.outputResult_nonEnglish(logCluL)
        print("Parsing done. [Time taken: {!s}]".format(datetime.now() - starttime))