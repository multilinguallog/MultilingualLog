...
def evaluatePA(groundtruth, result):
    ...
    for i in range(length):
        if result['template'][i] == groundtruth.loc[groundtruth['Content'] == result['log'][i]]['EventTemplate'].values[
            0]:
            correct += 1
...
class ModelParser():
    def __init__(self,
        ...
        # split candidate set
        self.log_test, self.log_cand, self.gt_test, self.gt_cand = self.splitCandidates(self.log_path, self.cand_ratio,
                                                                                        self.split_method)
    ...
    def splitCandidates(self, groundtruth_path, cand_ratio, method="random"):
        ...
        if method == "random":
            ...
            # split randomly
            log_test, log_cand, gt_test, gt_cand = train_test_split(log_list, groundtruth_template,
                                                                    test_size=cand_ratio, random_state=42)
        elif method == "DPP":
...
and put the template after <extraction> tag and between <START> and <END> tags."
        ...
        if self.evaluate:
            if not os.path.exists("DivLog_bechmark_result.csv"):
                df = pd.DataFrame(
                    columns=['Dataset', 'Parsing Accuracy', 'Precision Template Accuracy', 'Recall Template Accuracy',
                             'Grouping Accuracy'])
            else:
